...
namespace ConsoleExcelTest01
{
    class DataFormat01
    {
        ...
        public void Run()
        {
            ...
            SetValueAndFormat(workbook, cell7, new DateTime(2004, 5, 6), format.GetFormat("yyyy년m월d일"));
            cell7.SetCellValue(new DateTime(2004, 5, 6));

            //Chinese date string
            ICell cell8 = sheet.CreateRow(7).CreateCell(0);
            SetValueAndFormat(workbook, cell8, new DateTime(2005, 11, 6), format.GetFormat("yyyy년m월d일"));
            ...
            FileStream sw = File.Create(@"C:\00.Dev\temp\test.xlsx");
            workbook.Write(sw);
            sw.Close();
        }
    }
